...
const Index = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      } = await supabase.from('objectives').select('id').eq('deleted', false).lte('start_date', today).gte('end_date', today);
      ...
      } = await supabase.from('objectives').select('id').eq('deleted', true);
      ...
      if (objectiveIds.length > 0) {
        ...
        } = await supabase.from('key_results').select('id').eq('deleted', false).lte('start_date', today).gte('end_date', today).in('objective_id', objectiveIds);
        ...
        } = await supabase.from('initiatives').select('id').eq('deleted', false).eq('completed', false).lte('start_date', today).gte('end_date', today).in('objective_id', objectiveIds);
      ...
      if (objectiveIds.length > 0) {
        const {
          data: objectivesWithFrequency,
          error: freqError
        } = await supabase.from('objectives').select('id, check_in_frequency, start_date').eq('deleted', false).in('id', objectiveIds);
  ...
  return <div className="container mx-auto p-6">
      <div className="flex flex-col items-start mb-6">
        ...
        <p className="text-muted-foreground w-full">Your personal OKR management system. Track your objectives, key results, and initiatives to achieve your goals</p>
      </div>
      {isLoading ? <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
          ...
          <Card className="cursor-pointer" onClick={() => navigate("/check-in")}>
            <CardHeader>
          ...
          <Card className="cursor-pointer" onClick={() => navigate("/archive")}>
            <CardHeader>
