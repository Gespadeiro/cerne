...
const Archive = () => {
  ...
  const fetchObjectives = async () => {
    try {
      ...
      } = await supabase.from('objectives').select('*').eq('deleted', true).order('created_at', {
        ascending: false
      });
  ...
  return <div className="container p-6">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col items-start mb-6">
          ...
          <p className="text-muted-foreground w-full">Review and manage your archived objectives, key results, and initiatives. Restore items or delete them permanently</p>
        </div>
        <div className="mt-6 w-full">
          {objectives.length === 0 ? <div className="text-center py-20 bg-muted/20 rounded-lg">
              ...
              <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                  {objectives.map(objective => <tr key={objective.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
